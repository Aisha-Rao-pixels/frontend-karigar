    ...
    for r in refs:
        ...
        if r.get("referred_worker_id"):
            ...
            if w:
                ...
                worker_registered_at = w.get("created_at")
...
@api_router.get("/admin/referrals/export", response_class=PlainTextResponse)
async def export_referrals_csv(token: Optional[str] = None, user: dict = Depends(require_roles(*ADMIN_ROLES))):
    workers = await db.workers.find({"referral_code": {"$exists": True}}).to_list(10000)
    clicks = await db.referral_clicks.find().to_list(100000)
    click_counts: dict = {}
    for c in clicks:
    ...
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["Name", "Phone", "Referral Code", "Total Referred", "Registered", "Logged In Only", "Not Registered", "Total Earned (Rs)", "Paid (Rs)", "Pending (Rs)"])
    for worker in workers:
        code = worker.get("referral_code")
        if not code:
            continue
        ...
        paid = min(worker.get("manual_paid_rs", 0), total_earned)
        pending = max(total_earned - paid, 0)
        w.writerow([worker.get("full_name", ""), worker.get("phone", ""), code, total_referred, registered_count, account_created_count, not_registered, total_earned, paid, pending])
    return PlainTextResponse(content=buf.getvalue(), media_type="text/csv", headers={"Content-Disposition": "attachment; filename=referrals.csv"})
...
@api_router.get("/admin/permanent-delete-log")
async def list_permanent_delete_log(user: dict = Depends(require_roles(*ADMIN_ROLES))):
    if user.get("phone") not in PERMANENT_DELETE_PHONES:
